...
<h1>OMNI</h1>
<p align="center">
    <em><b>同じ出力を読み直すために払うのをやめましょう。</b>OMNI は繰り返しのバイトを取り出し可能なハンドルに変えます。エージェントが二度読むファイルで 97.2%、実際のコマンド 6,656 件で 14.9%。何も削除せず、何も捏造せず、どの数字もあなた自身のコーパスで再現できます。</em>
</p>
...
| **数字は実測であり願望ではない** | リリースバイナリで実トレース6,656件を再生。しかも 97.3% の呼び出しは削減ゼロで、その数字も公開する | [`ベンチマーク`](#ベンチマーク) |
...
**2026-08-04 から 08-11 UTC** をカバーする **6,656 件の実コマンド実行** を再生し、
リリースバイナリで実測しました。すべてモデルに届いた出力です。期間は数字の一部です。
`execution_traces` は7日で刈られるため、コーパスは測定の1週間後には消えています。

* ビルドとテストの出力は **76.9%**。最大のクラスであるファイル再読み込みはフィルタが
  **0.0%**、台帳が **25.0%** で、その差こそ台帳が存在する理由です。
* **呼び出しの 97.3% は何も削減しませんでした。** 残りがどれだけの価値かを示す数字なので
