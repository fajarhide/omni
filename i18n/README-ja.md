...
* ビルドとテストの出力は **87.9%**。最大のクラスであるファイル再読み込みはフィルタが
  **0.0%**、台帳が **24.6%** で、その差こそ台帳が存在する理由です。
* **呼び出しの 97.1% は何も削減しませんでした。** 残りがどれだけの価値かを示す数字なので
  公開しています。**この回では出力が大きくなった呼び出しは 1 件もありません。**
  かつて大きくなった 2 件は [#410](https://github.com/fajarhide/omni/issues/410) で直りました。
