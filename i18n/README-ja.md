...
<h1>OMNI</h1>
<p align="center">
    <em><b>同じ出力を読み直すために払うのをやめましょう。</b>OMNI は繰り返しのバイトを取り出し可能なハンドルに変えます。エージェントが二度読むファイルで 97.2%、実際のコマンド 5,984 件で、重い週は 69.6%、通常の週は 14.9%。何も削除せず、何も捏造せず、どの数字もあなた自身のコーパスで再現できます。</em>
</p>
...
| **数字は実測であり願望ではない** | リリースバイナリで実トレース5,984件を再生。すべての数字が対象コーパスと計測週を明示する | [`ベンチマーク`](#ベンチマーク) |
...
**2026-08-11 から 08-14 UTC** をカバーする **5,984 件の実コマンド実行** を再生し、
リリースバイナリで実測しました。すべてモデルに届いた出力です。期間は数字の一部です。
`execution_traces` は7日で刈られるため、コーパスは測定の1週間後には消えています。

* フィルタで **32.6%**、台帳を加えて **69.6%**。バイト量で最大のクラスである
  ファイル再読み込みはフィルタが **39.2%**、台帳まで含めて **89.6%** で、
  その差こそ台帳が存在する理由です。
* **数字より先にコーパスを読んでください。** この期間は特殊で、以下のすべてを押し上げて
  います。バイト単位で同一のペイロードが 286 グループあり、それだけで全バイトの
  **80.6%** を占め、5,984 件のうち 148 件が 64.7% を運んでいます。OMNI 自身を開発し
  計測していた週です。同じ計測を通常の作業の週に当てると **14.9%** になります。
* **バイトのある場所で効きます。** このコーパスで最大のクラスはファイルの再読み込みで、
  台帳はそこから **89.6%** を削ります。安全に削れるものがないとき、2 行の `git status` や
  後段が解析する JSON のような場合は、削減をでっち上げず出力をそのまま返します。
...
* **1コマンドあたり 21 ms**。ペイロードの大きさではなく履歴とともに増え、205 MB の
  データベースでは 61 ms です。
* **上の数値はすべてコマンドあたりのバイト数であり、請求額とは別物です。** 課金される入力トークンはおおむねターン数×プレフィックスサイズで決まるため、ペイロードを短くしても、それがターンを一つ減らして初めて支払いに効きます。セッション全体をエンドツーエンドで測ると、削減幅は平均ではこの表より大きく、しかし個々のセッションで保証されるものではなく、請求がまったく下がらなかった実行もあります。
