...
<h1>OMNI</h1>
<p align="center">
    <em><b>1万行のターミナルノイズを読ませるために Claude に課金するのは、もう終わりです。</b>ある開発者の実際の1週間で、OMNI はビルドとテストの出力を 88%、エージェントが読み直した分の4分の1を削り、全体では 15.7% でした。残り 97% の呼び出しはそのまま通します。失われるものは何もなく、結果を捏造することもありません。</em>
</p>
...
<b>
ビルドとテスト 88% &middot; ファイル再読み込み 25% &middot; 全体で 15.7% &middot; 1コマンドあたり 21 ms &middot; 7,095 件中、出力が大きくなった呼び出しが 2 件あり、それも公開 &middot; 切った分はバイト単位で復元可能 &middot; セッション横断メモリ </b>
...
* **ノイズのあるところでは、フィルタがほぼ全部を取ります。** ビルドとテストの出力は
  87.9%、セッション台帳を含めれば 92.3% です。ノイズのないところでは何も取らず、
  `kubectl get pods` の表は 0% です。どの行もデータだからです。
* **台帳はフィルタが届かないところに届きます。** ファイルの再読み込みは 1.54 MB で
  最大のクラスですが、フィルタはその 0.0% しか取れません。すでにエージェントに見せた
  行を返すことで 24.6% になります。
* **97.1% の呼び出しは何も削減せず**、出力をそのまま返しました。削減分はすべて残りの
  2.9% から来ています。
* **7,095 件中 2 件は出力が大きくなりました。** 丸めて消さずに報告します
  ([#398](https://github.com/fajarhide/omni/issues/398))。
* **構成全体では 15.7% 減。** うちフィルタが 5.2%、残りは台帳です。トークンで数えると
  フィルタ単独で 5.0% です。
...
| 問い | なぜ効くか | OMNI |
|---|---|---|
| **何も削減しなかった**呼び出しの割合は | すべてのコマンドで削減するツールは、必要な出力を要約している | **97.1%**、公開しています |
| 出力が**大きくなった**呼び出しはあるか | マーカーやヘッダはバイトを食うが、裏目に出た分を誰も報告しない | **7,095 件中 2 件**、issue 番号もあります |
| どの**母集団**を測ったか | どのモデルも読まないターミナルバイトを数えれば、数字はただで膨らむ | モデルに届いた分だけ。ターミナル行を含むコーパスで最後に測ったときは、そう言うことで 36 ポイント失いました |
