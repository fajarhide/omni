...
<h1>OMNI</h1>
<p align="center">
    <em><b>에이전트는 이미 본 출력에 두 번 값을 치릅니다.</b> OMNI는 대신 다시 꺼낼 수 있는 핸들을 돌려줍니다. 두 번 읽는 파일에서 <b>97.2%</b>, 코퍼스 전체의 파일 읽기에서 <b>89.6%</b>. 아무것도 삭제하지 않고, 아무것도 지어내지 않으며, 모든 숫자는 당신의 기록에서 그대로 재현됩니다.</em>
</p>
...
| **숫자는 측정된 것이지 희망이 아님** | 릴리스 바이너리에서 실제 트레이스 5,984건 재생. 모든 수치가 대상 코퍼스와 측정 주간을 함께 밝힘 | [`벤치마크`](#벤치마크) |
...
**2026-08-11부터 08-14 UTC까지**를 아우르는 **5,984건의 실제 명령 실행**을 재생해
릴리스 바이너리에서 측정했습니다. 모두 모델에 도달한 출력입니다. 기간은 숫자의
일부입니다. `execution_traces`는 7일 뒤 정리되므로, 코퍼스는 측정 일주일 뒤에
사라집니다.

* 필터가 **32.6%**, 원장까지 더하면 **69.6%**. 바이트 기준 가장 큰 분류인 파일
  재열람은 필터가 **39.2%**, 원장까지 **89.6%**이며, 그 간극이 원장이 존재하는
  이유입니다.
* **숫자보다 코퍼스를 먼저 읽으세요.** 이 구간은 특이하고 아래의 모든 수치를 부풀립니다.
  바이트 단위로 동일한 페이로드가 286개 그룹 있어 전체 바이트의 **80.6%**를 차지하고,
  5,984건 중 148건이 그중 64.7%를 나릅니다. OMNI 자체를 만들고 계측하던 한 주였습니다.
  같은 방식으로 평범한 한 주를 재면 **14.9%**가 나옵니다.
* **바이트가 있는 곳에서 작동합니다.** 이 코퍼스에서 가장 큰 클래스는 파일 재읽기이며,
  원장은 거기서 **89.6%**를 덜어냅니다. 안전하게 덜어낼 것이 없을 때, 두 줄짜리
  `git status`나 뒤 단계가 파싱할 JSON 같은 경우에는 절약을 지어내지 않고 출력을 그대로
  돌려줍니다. **이번 측정에서 출력이 더 커진 호출은 한 건도 없습니다.**
