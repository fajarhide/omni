...
> 터미널과 LLM 사이의 고성능 시맨틱 필터 역할을 합니다. 노이즈가 많은 로그를 지능적으로 정제하고, 상태를 캐싱하며, 토큰 예산을 관리함으로써 OMNI는 에이전트가 집중력을 유지하고 환각(hallucination)을 줄이며 루프를 결함 없이 실행하도록 보장합니다. 이 모든 과정을 통해 **API 비용을 최대 90%까지 절감**합니다.
> 
