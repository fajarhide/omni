...
<b>
모델에 도달하는 바이트 43.3% 감소, 실제 명령 9,965건에서 실측 &middot; 세션 간 메모리 &middot; 포맷 안전 &middot; 항상 복원 가능 &middot; 페일 오픈, 지어내지 않음 &middot; 재현 가능한 숫자 </b>
...
솔직한 헤드라인 숫자입니다. 한 개발자의 실제 사용에서 재생한 **9,965건의 실제 명령
실행**에 대해 릴리스 바이너리에서 측정했습니다.

* 모델에 도달하는 바이트가 전체 구성에서 **43.3% 감소** (40.1 MB에서 22.7 MB로).
* **그중 90.0%의 호출은 아무것도 절약하지 못했습니다.** OMNI는 출력을 그대로 돌려주며
  **0** 바이트도 더하지 않았습니다. 절감은 전부 실제로 노이즈가 있었던 나머지 10.0%에서
  나옵니다.
...
대가는 기록과 함께 커집니다. 496바이트 `git status`는 새 데이터베이스에서 약 21 ms,
205 MB 데이터베이스에서 약 61 ms가 걸립니다. 16.5 KB `cargo test`는 약 25 ms입니다.
예산에 넣어두세요.
