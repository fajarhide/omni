...
<h1>OMNI</h1>
<p align="center">
    <em><b>터미널 노이즈 1만 줄을 읽히려고 Claude에 비용을 내는 일은 이제 그만두세요.</b> 한 개발자의 실제 한 주 동안 OMNI는 빌드와 테스트 출력의 88%, 에이전트가 다시 읽은 것의 4분의 1을 잘라냈고, 전체로는 15.7%였습니다. 나머지 97%의 호출은 그대로 통과합니다. 잃어버리는 것은 없고, 결과를 지어내지도 않습니다.</em>
</p>
...
<b>
빌드와 테스트 88% &middot; 파일 재열람 25% &middot; 전체 15.7% &middot; 명령당 21 ms &middot; 7,095건 중 출력이 커진 호출 2건, 그것도 공개 &middot; 잘라낸 모든 것을 바이트 단위로 복원 &middot; 세션 간 메모리 </b>
...
* **노이즈가 있는 곳에서는 필터가 거의 전부를 가져갑니다.** 빌드와 테스트 출력은
  87.9%, 세션 원장까지 세면 92.3%입니다. 노이즈가 없는 곳에서는 아무것도 가져가지
  않아 `kubectl get pods` 표는 0%입니다. 모든 행이 데이터이기 때문입니다.
* **원장은 필터가 닿지 못하는 곳에 닿습니다.** 파일 재열람이 1.54 MB로 가장 큰
  분류인데 필터는 그중 0.0%만 가져가고, 에이전트에게 이미 보여준 줄을 돌려주는 것이
  24.6%를 가져갑니다.
* **호출의 97.1%는 아무것도 절약하지 못하고** 출력을 그대로 넘겼습니다. 절약분은
  전부 나머지 2.9%에서 나옵니다.
* **7,095건 중 2건은 출력이 더 커졌습니다.** 반올림해 없애지 않고 보고합니다
  ([#398](https://github.com/fajarhide/omni/issues/398)).
* 전체 구성에서 **바이트 15.7% 감소**. 그중 필터가 5.2%, 나머지는 원장입니다. 토큰
  기준으로 필터만 세면 5.0%입니다.
...
| 질문 | 왜 중요한가 | OMNI |
|---|---|---|
| **아무것도 절약하지 못한** 호출의 비율은? | 모든 명령에서 절약하는 도구는 당신에게 필요한 출력을 요약하고 있는 것 | **97.1%**, 공개합니다 |
| 출력을 **더 크게** 만든 호출이 있었나? | 마커와 헤더는 바이트를 먹지만, 역효과가 난 쪽을 아무도 보고하지 않는다 | **7,095건 중 2건**, 이슈 번호도 있습니다 |
| 어떤 **모집단**을 쟀나? | 어떤 모델도 읽지 않는 터미널 바이트를 세면 숫자가 공짜로 부푼다 | 모델에 도달한 것만. 터미널 줄이 있는 코퍼스를 마지막으로 쟀을 때 그렇게 말하느라 36포인트를 잃었습니다 |
