...
* 构建与测试输出 **87.9%**。最大的一类是文件重读，过滤器拿走 **0.0%**，账本拿走
  **24.6%**，这个落差正是账本存在的理由。
* **97.1% 的调用一点没省**，我们照样公布，因为这个数字才说明剩下的值多少。**这一轮没有一次调用让输出变大**，
  曾经变大的那 2 次已由 [#410](https://github.com/fajarhide/omni/issues/410) 修好。
