<div align="center">
  ...
  **为 AI 智能体提供降噪上下文与长期记忆 —— 有损，但永远可还原，且绝不编造结果。别再为让 Claude 读一万行终端噪音而付费。**

  **真实命令组合下 Token 减少 58.9% · 跨会话记忆 · 格式安全 · 永远可还原 · 失败即放行，绝不编造 · 可复现的数字**
...
> 它作为终端和 LLM 之间的高性能语义过滤器运行。通过智能提取嘈杂的日志、缓存状态并管理 Token 预算，OMNI 确保您的智能体保持专注、减少幻觉并完美执行循环。在发布版二进制上重放 1,810 条真实 trace 的实测结果是：到达模型的字节数**减少 58.9%**（15.0 MB → 6.2 MB）——同时**有 63.6% 的调用一点也没省下**，这个数字我们同样公开。
> 
...
- **真实命令组合下 Token 减少 58.9%**: 这是实测而非愿景——在发布版二进制上重放了 1,810 条真实 trace。但请注意：**63.6% 的调用一点也没省下** —— OMNI 原样返回输出，一个字节也不添加。全部节省都来自确实存在噪音的另外 36.4%。
...
OMNI 不仅仅是一个正则表达式脚本；它是一个用 Rust 编写的高性能 **语义信号引擎**。那么，它如何在约 100 毫秒内对嘈杂输出进行削减，又如何在无从削减时选择什么都不做呢？
