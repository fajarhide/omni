...
<h1>OMNI</h1>
<p align="center">
    <em><b>别再为让 Claude 读一万行终端噪音而付费。</b>在某位开发者真实的一周里，OMNI 砍掉了 88% 的构建与测试输出、智能体重读内容的四分之一，整个组合上是 15.7%。其余 97% 的调用原样通过。没有任何东西会丢失，它也绝不编造结果。</em>
</p>
...
<b>
构建与测试 88% &middot; 文件重读 25% &middot; 整体 15.7% &middot; 每条命令 21 ms &middot; 7,095 次调用中有 2 次让输出变大，我们照说 &middot; 每一处裁剪都可按字节还原 &middot; 跨会话记忆 </b>
...
| **数字是测出来的，不是喊出来的** | 在发布版二进制上回放 7,095 条真实 trace，而且 97.1% 的调用一点没省，这个数字我们同样公开 | [`基准测试`](#基准测试) |
...
在发布版二进制上，回放 **7,095 次真实命令执行**测得，覆盖 **2026-08-03 到 08-10
UTC**，每一次都是抵达模型的输出。时间窗口是这个数字的一部分：`execution_traces`
七天后就会被清理，所以一份语料在测完一周后就不存在了。

* **有噪音的地方，过滤器几乎全拿走。** 构建与测试输出是 87.9%，把会话账本算进来是
  92.3%。没有噪音的地方它什么都不拿，`kubectl get pods` 的表格是 0%，因为其中每一
  行都是数据。
* **账本够得到过滤够不到的地方。** 文件重读是最大的一类，1.54 MB，过滤器只拿走其中
  0.0%，而把智能体已经看过的行交还回去能拿走 24.6%。
* **97.1% 的调用一点没省**，原样把输出交回去。所有节省都来自剩下的 2.9%。
* **7,095 次里有 2 次让输出变大**，我们照实报告而不是四舍五入抹掉
  （[#398](https://github.com/fajarhide/omni/issues/398)）。
* 整个组合上**字节减少 15.7%**，其中过滤器占 5.2%，其余来自账本。按 token 计，单
  过滤器是 5.0%。
...
| 问题 | 为什么要紧 | OMNI |
|---|---|---|
| 有多大比例的调用**一点没省**？ | 一个对每条命令都能省的工具，是在摘要你需要的输出 | **97.1%**，我们公布 |
| 有没有调用让输出**变大**？ | 标记和表头都要占字节，但没人报告适得其反的那些 | **7,095 次里 2 次**，而且都有 issue 编号 |
| 测的是哪个**样本**？ | 把没有模型读过的终端字节算进去，数字白涨 | 只算到达模型的部分；上一次语料里还有终端行时，这么说让我们少了 36 个点 |
