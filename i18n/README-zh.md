...
<h1>OMNI</h1>
<p align="center">
    <em><b>别再为重读同一份输出付费。</b>OMNI 把重复的字节换成可取回的句柄：智能体读两遍的文件省 97.2%，5,984 条真实命令上，繁重的一周省 69.6%，平常的一周省 14.9%。什么都不删除，什么都不编造，每个数字都能在你自己的语料上重放。</em>
</p>
...
| **数字是测出来的，不是喊出来的** | 在发布版二进制上回放 5,984 条真实 trace，每个数字都注明其语料与测量周 | [`基准测试`](#基准测试) |
...
在发布版二进制上，回放 **5,984 次真实命令执行**测得，覆盖 **2026-08-11 到 08-14
UTC**，每一次都是抵达模型的输出。时间窗口是这个数字的一部分：`execution_traces`
七天后就会被清理，所以一份语料在测完一周后就不存在了。

* 过滤器拿走 **32.6%**，加上账本是 **69.6%**。按字节算最大的一类是文件重读，
  过滤器拿走 **39.2%**，加上账本是 **89.6%**，这个落差正是账本存在的理由。
* **先读语料，再读数字。** 这个时间窗口并不寻常，它抬高了下面的每一个数字：有 286 组
  逐字节相同的载荷，单是它们就占了全部字节的 **80.6%**，而 5,984 次调用里有 148 次
  承载了其中 64.7%。那是专门开发和测试 OMNI 的一周。同一套方法用在平常工作的一周上，
  读数是 **14.9%**。
* **它在字节所在之处发力。** 本语料中最大的一类是文件重复读取，账本从中削去 **89.6%**。当没有可安全削减的内容时，比如两行的 `git status`，或后续步骤要解析的 JSON，OMNI 原样交还输出，而不是编造一个节省数字。**本次测量中没有一次调用让输出变大。**
...
* **上面每个数字都是每条命令的字节数，跟你的账单不是一回事。** 计费的输入 token 大致等于轮次数乘以前缀大小，所以缩短载荷只有在同时减少一个轮次时才真正省钱。按整个会话端到端测量，平均节省比这张表更大，但在任何单个会话上都没有保证，也有账单完全没下降的运行。
