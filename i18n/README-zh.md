...
<b>
到达模型的字节减少 43.3%，实测自 9,965 条真实命令 &middot; 跨会话记忆 &middot; 格式安全 &middot; 永远可还原 &middot; 失败即放行，绝不编造 &middot; 可复现的数字 </b>
...
* 到达模型的字节数，在整个组合上**减少 43.3%**（40.1 MB 到 22.7 MB）。
* **其中 90.0% 的调用一点也没省。** OMNI 把输出原样递回，**没有**多加一个字节。
  全部节省都来自另外 10.0%，那里确实有噪音可砍。
...
增长：496 字节的 `git status` 对着全新数据库约 21 ms，对着 205 MB 的数据库约 61 ms；
16.5 KB 的 `cargo test` 约 25 ms。请把它算进预算。
