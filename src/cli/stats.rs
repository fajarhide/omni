...
pub fn run_gain(_args: &[String], store: &Store) -> Result<()> {
    let periods = store.multi_period_stats()?;
    println!("\n{}", "OMNI Gain".bold().bright_white());
    ...
    for (label, count, input, output) in &periods {
        ...
        if label == "Today" || label == "Last 7 Days" {
            let tokens_saved = format_tokens(input.saturating_sub(*output));
            ...
            println!(
                "{:<11} {:>3} commands │ ~{:<6} tokens saved │ {:.0}% avg compression",
                format!("{}:", label).bright_black(),
                count,
                tokens_saved.bright_green(),
                reduction_pct
            );
            if label == "Today" {
                today_found = true;
    ...
    if !today_found {
        println!(
            "{}",
            "No token savings recorded yet today."
                .bright_black()
                .italic()
        );
    }
    ...
    if !top.is_empty() {
        println!("\nTop savings:");
        for (cmd, count, pct) in top {
            println!(
                "  {:<16} {:.0}% avg compression ({:>2}x)",
                shorten_command(&cmd, 16).bright_cyan(),
                pct,
                count
            );
        }
    ...
    // Retrieve rate for context
    let overall_retrieve_rate = store.get_retrieve_rate("", 7) * 100.0;
    println!(
        "\nRetrieve rate: {:.1}% {}",
        overall_retrieve_rate,
        if overall_retrieve_rate < 15.0 {
            "healthy".bright_green()
        } else {
            "high (over-compressing?)".bright_red()
        }
    );
