...
fn is_vitest_output(lines: &[&str]) -> bool {
    lines.iter().any(|l| l.contains("vitest") || l.contains("VITE v") || l.contains("Test Files") || l.contains("Tests  "))
}

fn is_tsc_output(lines: &[&str]) -> bool {
    lines.iter().any(|l| l.contains("error TS") || l.contains("tsc --") || l.contains("Found errors") || l.contains("Found ") && l.contains(" error"))
}

fn is_playwright_output(lines: &[&str]) -> bool {
    lines.iter().any(|l| l.contains("playwright") || l.contains("[chromium]") || l.contains("[firefox]") || l.contains("Running ") && l.contains(" tests"))
}

fn is_eslint_output(lines: &[&str]) -> bool {
    lines.iter().any(|l| l.contains("eslint") || l.contains(" problems (") || (l.contains("error") && l.contains("@typescript-eslint")))
}

fn is_prettier_output(lines: &[&str]) -> bool {
    lines.iter().any(|l| l.contains("prettier") || l.contains("checking ") && l.contains(" files") || l.contains("reformatted ") && l.contains(" files"))
}
...
fn distill_vitest(input: &str) -> String {
    ...
    let fail_count_display = if failed_tests > 0 { failed_tests } else { unique_fails.len() as u32 };
...
fn distill_tsc(input: &str) -> String {
    ...
    for line in input.lines() {
        ...
        if let Some(ts_idx) = t.find("error TS") {
            ...
            // "Found 5 errors"
            if let Some(num) = t.split_whitespace().nth(1).and_then(|s| s.parse::<u32>().ok())
                && total_errors == 0 {
...
fn distill_playwright(input: &str) -> String {
    ...
    for line in &lines {
        ...
        if t.ends_with("passed") || (t.contains("passed (") && t.ends_with(")")) {
            if let Some(num) = t.split_whitespace().next().and_then(|s| s.parse::<u32>().ok()) {
                passed = num;
            }
        } else if (t.ends_with("failed") || (t.contains("failed (") && t.ends_with(")")))
            && let Some(num) = t.split_whitespace().next().and_then(|s| s.parse::<u32>().ok()) {
                failed = num;
            }
    }
...
fn distill_eslint(input: &str) -> String {
    ...
    for line in input.lines() {
        ...
        if !t.contains(" error ") && !t.contains(" warning ") && (t.contains('/') || t.contains('\\')) && !t.contains(' ') {
    ...
    let mut out = format!("eslint: {} errors, {} warnings in {} files", total_errors, total_warnings, files_affected.len());
    ...
    if !by_rule.is_empty() {
        ...
        out.push_str("\n  top rules: ");
        let rules_str: Vec<String> = sorted.iter().take(3).map(|(r, c)| format!("{}: {}", r, c)).collect();
...
fn distill_prettier(input: &str) -> String {
    ...
    for line in input.lines() {
        ...
        if t.contains("reformatted ") && t.contains(" files")
    ...
    format!("prettier: {} files reformatted, {} unchanged", reformatted, unchanged)
}
